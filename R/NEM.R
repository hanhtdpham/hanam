#' Fitting a network effects model
#'
...
NEM <- function(y, X, A,
  ...
  # Initialize
  if(is.null(init_vec)){
    fit2sls  <- lnam2sls_effect(y, X, A)
    init_vec <- c(fit2sls$coefs[1:p], fit2sls$s2, fit2sls$coefs[p+1])
  }
  NEM_result <- optim(init_vec,
                      fn = lPostNEM, gr = gradlPostNEM,
                      method = "L-BFGS-B",
  ...
  covar <- chol2inv(chol(nearPD(-NEM_result$hessian)$mat))
  se    <- sqrt(diag(covar))

  coefs <- data.frame(estimates = c(beta  = NEM_result$par[1:p],
                                    rho   = NEM_result$par[p+2]),
                      se        = c(beta  = se[1:p],
                                    rho   = se[p+2]))
  ...
  logLik <- ll_nam(NEM_result$par, y, X, A, modeltype = "NEM")

  ret <- list(init        = c(beta  = init_vec[1:p],
                              s2    = init_vec[p+1],
                              rho   = init_vec[p+2]),
              coefs       = coefs,
              s2          = NEM_result$par[p+1],
              s2_se       = se[p+1],
              covar       = covar,
              convergence = NEM_result$convergence,
              logLik      = logLik)
  class(ret) <- "NAM"
  return(ret)
}
...
gradlPostNEM <- function(theta, y, X, A,
                         mu_b = rep(0, ncol(X)),
                         s2_b = 2.25^2,
                         a_s2 = 2,
                         b_s2 = 2,
  ...
  # Precompute
  mu     <- ((diag(1, n)-rho*A)%*%y - X%*%beta)

  # Derivative of log posterior
  GradBeta    <- crossprod(X, mu)/sigmasq - (beta-mu_b)/s2_b
  Gradsigmasq <- -(0.5*(n+a_s2) + 1)/sigmasq +
    (0.5*drop(crossprod(mu)) + 0.5*b_s2)/(sigmasq^2)
  GradRho     <- -sum(diag(A%*%M)) +
    crossprod(A%*%y, mu)/sigmasq - (rho - mu_rho)/(sd_rho^2)

  out <- c(GradBeta, Gradsigmasq, GradRho)
  return(unlist(out))
}

lPostNEM <- function(theta, y, X, A,
                     mu_b = rep(0, ncol(X)),
                     s2_b = 2.25^2,
                     a_s2 = 2,
                     b_s2 = 2,
